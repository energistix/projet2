class Game {
  ...
  removeAllImages() {
    ...
    for (let i = this.gameView.children.length - 1; i >= 0; i--) {
      this.gameView.children[i].parentNode.removeChild(this.gameView.children[i])
    }
  }
  ...
  updateScore() {
    this.scoreDisplay.innerText = `Pairs trouvées : ${this.stats.foundPairs}/${this.stats.totalPairs}-${Math.round(
      (this.stats.foundPairs / this.stats.totalPairs) * 100
    )}%
        Nombre de tours : ${this.stats.turns}${this.won ? "\nPartie gagné !!" : ""}`
  }
